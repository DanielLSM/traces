...
class SchedulerEDF(FleetManagerBase):
  ...
  def save_checks_to_xlsx(self):
    ...
    for aircraft in self.global_schedule.keys():
      for _ in range(len(self.global_schedule[aircraft]['due_dates'])):
        ...
        dict1['START'].append(self.global_schedule[aircraft]['due_dates'][_].date().isoformat())
        dict1['END'].append(self.global_schedule[aircraft]['due_dates'][_].date().isoformat())
    ...
    for aircraft in tqdm(self.global_schedule_tasks.keys()):
      for day in self.global_schedule_tasks[aircraft]['a_check_tasks'].keys():
        for item in self.global_schedule_tasks[aircraft]['a_check_tasks'][day]:
          item_idxs = df[(df['A/C'] == aircraft) & (df['ITEM'] == item)].index.values.astype(int)
  ...
  def _add_to_global_schedule(self, schedule_partial):
    for aircraft in self.fleet.aircraft_info.keys():
      ...
      self.global_schedule[aircraft]['DY'].append(schedule_partial[aircraft]['A_INITIAL']['DY'])
      self.global_schedule[aircraft]['FH'].append(schedule_partial[aircraft]['A_INITIAL']['FH'])
      self.global_schedule[aircraft]['FC'].append(schedule_partial[aircraft]['A_INITIAL']['FC'])
  ...
  def get_max_slots(self, due_date, key=None):
    check_types = ['a-type', 'c-type']
    slots = [self.calendar.calendar[due_date]['resources']['slots'][check] for check in check_types]
    max_slots = max(slots)
    return max_slots
  ...
  def get_domain(self, date_start, date_end, check_type='a-type', variable=None):
    ...
    while due_date <= date_end:
      if self.calendar.calendar[due_date]['allowed']['public holidays'] and self.calendar.calendar[
  ...
  def get_schedule_stats(self, assignment, context):
    ...
    for aircraft in assignment.keys():
      ...
      schedule_partial[aircraft]['A_INITIAL']['last_due_date'] = context[aircraft]['A_INITIAL'][
          'last_due_date']
  ...
  def _compute_inital_context(self):
    ...
    for check in checks.keys():
      for aircraft in self.fleet.aircraft_info.keys():
        ...
        due_dates[aircraft][check]['due_date'], due_dates[aircraft][check]['waste'], due_dates[
            aircraft][check]['last_due_date'] = self.compute_next_due_date(self.start_date,
                                                                           self.end_date,
                                                                           aircraft,
                                                                           DY_i=DY_i,
                                                                           FH_i=FH_i,
...
###############################################################################
  ...
  def _extend_process_a_tasks(self, aircraft):
    ...
    for a_item_code in a_items_codes:
      ...
      last_exec_date = task_a['LAST EXEC DT'] if task_a['LAST EXEC DT'] else task_a['LIMIT EXEC DT']
